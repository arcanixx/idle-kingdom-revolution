...
>
> **Terminologia (v2.0, 2026-06-27):** "Frakcja"/"faction" poniżej, gdzie
> odnosi się do Human/Elf/Orc/Undead/Demon/Celestial, to właściwie **Rasa**
> — patrz `00_FOUNDATION/00_ART_DIRECTION.md` → RACE, FACTION, AND CLASS.
> Liczby i struktura poniżej są niezmienione, tylko nazewnictwo poprawione.
...
| Unit Portraits (Hero: 1 race × 7 classes × 6 rarity × 3 states × 2 genders) | 252 | 512×512 | 1-3 |
...
**Rasy (6):** Human, Elf, Orc, Undead, Demon, Celestial
...
**Character Types (3):** Hero (party members, 1 race+faction active), Generic Unit (3 facial variants per class, Common idle), NPC (unique face)
**Płeć (2):** Male, Female (per character choice)

**Docelowo (na 1 aktywną rasę+frakcję):**
- Hero: 7 classes × 6 rarity × 3 states × 2 genders = **252 pliki**
- Generic Unit: 6 races × 7 classes × 3 faces = **126 plików** (Common, idle)
- NPC: per story demand (unique face, any class/race/faction)

> **Dlaczego 252, a nie 36?** Bo gracz ma drużynę (party) – może mieć do 7 różnych klas w swoim składzie.
> Każda z nich otrzymuje pełną progresję rarity. Tylko 1 rasa+frakcja jest aktywna na playthrough.
> Generic Units (3 twarze = 3 różne wyglądy twarzy) pokrywają pozostałe 5 ras + wrogów.
...
### Emotion Portraits (podstawowy zestaw – 15 emocji × 1 rasa)
...
- 28 × Unit Portraits (4 klasy × 1 rasa, wszystkie rarity)
- Reszta Item Icons
...
**FAZA 3 (Pełnia):**
- Hero (1 race+faction × 7 classes × 6 rarity × 3 states × 2 genders = 252 pliki)
- Generic Unit Variants (6 races × 7 classes × 3 faces = 126)
- Wszyscy bossowie (6)
- Wszystkie UI (8)
- Avatary (42)
- Pełen zestaw Emotion Portraits (dla wszystkich ras)
