...
> Pełne definicje i prompty: zobacz pliki w `04_CATEGORIES/` (dawniej `01_*` do `07_*` w tym folderze).
>
> **Uwaga (2026-06-27):** Ten plik został przeniesiony do `05_REFERENCE/` w ramach
...
1. Generujesz asset wg promptu z odpowiedniego pliku w `04_CATEGORIES/` (np. `00_BACKGROUNDS.md` itd.)
2. Po przejściu pipeline'u (`03_PIPELINE/03_WORKFLOW_AND_TOOLS.md`) i wylądowaniu w `public/assets/{kategoria}/` – zaznacz `[x]` tutaj.
3. Jeśli dodajesz coś nowego, czego nie ma w tej liście (np. nową frakcję, nowy boss) – dopisz wiersz tutaj ORAZ zaktualizuj odpowiedni plik w `04_CATEGORIES/`, żeby dokumentacja i checklist się nie rozjeżdżały.
...
## 1. Battle Backgrounds (`04_CATEGORIES/00_BACKGROUNDS.md`)
...
## 2. Unit Portraits (`04_CATEGORIES/01_UNIT_PORTRAITS.md`)
...
**Orc** (klasy unikalne – patrz uwaga w `04_CATEGORIES/01_UNIT_PORTRAITS.md`):
- [ ] orc_warrior_base
- [ ] orc_tank_base
- [ ] orc_berserker_base
- [ ] orc_shaman_base
...
> Uwaga dotycząca generowania: Wszystkie 6 rarity dla danej klasy generujemy w jednym obrazie produkcyjnym 2x3. Kolejność slotów: Common, Uncommon, Rare, Epic, Legendary, Mythic. Wycinamy je skryptem po zaakceptowaniu QA.
...
> To NIE są stany (idle/hit/attack) ani emocje (portrety). Tylko 3 różne seedy w prompcie = 3 różne twarze.
...
## 3. Item Icons (`04_CATEGORIES/02_ITEM_ICONS.md`)
...
## 4. Boss Creatures (`04_CATEGORIES/03_BOSS_CREATURES.md`)
...
## 5. UI Elements (`04_CATEGORIES/04_UI_ELEMENTS.md`)
...
## 6. Emotion Portraits (`04_CATEGORIES/05_EMOTION_PORTRAITS.md`)
...
## 7. Environment Effects (`04_CATEGORIES/06_ENVIRONMENT_EFFECTS.md`)
