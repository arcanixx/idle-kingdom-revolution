...
> `00_FOUNDATION/00_ART_DIRECTION.md` → RACE, FACTION, AND CLASS. Motyw
> (np. lew, liść) jest atrybutem tej Rasy's domyślnej **Faction**.
...
- **Detale:** Widoczny detal zbroi/ubioru, motyw rasy/frakcji.
...
|-----------|--------|------|-----------------|
| **Core** (P0) | Neutral, Happy, Angry, Sad, Surprised, Scared, Serious | Podstawowe emocje, zawsze potrzebne | **Generuj od razu** dla wszystkich ras+klas |
...
|------|-----------------|----------|-----------|
| General | Zawsze, jako standardowy zestaw | Wszystkie rasy + klasy | P1 -- generuj w pierwszej kolejności |
| Special | Tylko na zgłoszenie (story, event, quest) | Konkretna rasa + klasa, tylko potrzebujące | P3 -- generuj na życzenie |

**Zasada:** Nie generuj Special stanów dla wszystkich ras/klas. Generuj je tylko wtedy, gdy dana scena fabularna lub event wymaga konkretnego stanu u konkretnej postaci.
...
Fantasy 3D render portrait of a {emotion} {race} {class}, {rarity} quality, {emotion_description}, half-body shot, {pose_hint}, soft studio lighting, warm key light, detailed fantasy armor with faction motif ({faction_motif}), transparent background, game portrait, high detail, 4K --ar 1:1 --style raw --v 6
...
**Nazewnictwo:** {race}_{class}_{emotion}.webp
...
- Zawsze masz Core Emotions dla każdej klasy/rasy. Reszta to "bonus".

Implementacja w kodzie:

```tsx
function getEmotionAsset(race: string, cls: string, emotion: string): string {
  const path = `/assets/portraits/$ {race}/$ {race}_$ {cls}_$ {emotion}.webp`;
  ...
  return `/assets/portraits/$ {race}/$ {race}_$ {cls}_$ {coreFallback[emotion] || "neutral"}.webp`;
}
```
...
- **Generuj partiami:** Najpierw General dla Human (najważniejsza rasa), potem General dla pozostałych. Special stany generuj dopiero na życzenie.
...
- **Stan wyjątkowy:** Berserk jest Special, ale ma sens głownie dla Orc i NPC -- nie generuj dla wszystkich ras.
