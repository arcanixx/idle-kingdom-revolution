...
describe('Battle Engine', () => {
  ...
  const baseUnits: Unit[] = [{
    ...
    equipment: { weapon: undefined, armor: undefined, helmet: undefined, accessory: undefined }, xp: 0
  }];

  describe('startBattle', () => {
    it('initializes state with correct wave count', () => {
      const state = startBattle(baseField, baseUnits, { front: [{ unit_id: 'a', row: 0, col: 0 }], back: [null, null, null, null] });
    ...
    it('handles empty formation', () => {
      const state = startBattle(baseField, baseUnits, { front: [null, null, null], back: [null, null, null, null] });
      expect(state.playerUnits).toHaveLength(0);
    });

    it('filters out inactive units', () => {
      const inactiveUnits = [{ ...baseUnits[0], isActive: false }];
      const state = startBattle(baseField, inactiveUnits, { front: [{ unit_id: 'a', row: 0, col: 0 }], back: [null, null, null, null] });
      expect(state.playerUnits).toHaveLength(0);
    });

    it('sets default rewards when not provided', () => {
      const fieldNoRewards = { ...baseField, id: "2", rewards: null as any };
      const state = startBattle(fieldNoRewards, baseUnits, { front: [{ unit_id: 'a', row: 0, col: 0 }], back: [null, null, null, null] });
      expect(state.rewards.gold).toBe(50);
      expect(state.rewards.xp).toBe(30);
    });

    it('handles null formation cells', () => {
      const state = startBattle(baseField, baseUnits, { front: [null, { unit_id: 'a', row: 0, col: 1 }], back: [null, null, null, null] });
