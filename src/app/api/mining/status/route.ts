...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: mining, error } = await supabase.from("player_mining").select("*").eq("player_id", player.id).maybeSingle();
    ...
    if (mining?.expedition_ends_at) {
      ...
      if (end > now) { expActive = true; remainSec = Math.floor((end - now) / 1000); }
    }
    return jsonResponse({
